...
class CapsNet:
    def __init__(self, is_training=False, routing = 0, dcap_init = 0):
        ...
        with self.graph.as_default():
            if is_training:
                ...
                self.x = tf.placeholder(tf.float32, [None, 32, 32, 1], name='input')
            self.input = tf.reshape(self.x, [conf.batch_size, datasize, datasize, 3])
            assert self.input.get_shape() == (conf.batch_size, datasize, datasize, 3)
            if dcap_init == 0:
    ...
    def build_model_capnets(self, is_training=False):
        with tf.variable_scope("cap_nets") as scope:
            with tf.variable_scope('conv1_layer'):
                ...
                assert conv1.get_shape() == (conf.batch_size, 24, 24, 256)
            ...
            with tf.variable_scope('primary_caps'):
                ...
                assert pcaps.get_shape() == (conf.batch_size, 32*8*8, 8)
    ...
    def build_model(self, is_training=False, routing = 0):
        ...
        with tf.variable_scope('reconstruction'):
            ...
            # assert self.decoded.get_shape() == (conf.batch_size, datasize, datasize)
    ...
    def primary_caps(self, inputs):
        # Validate inputs
        assert inputs.get_shape() == (conf.batch_size, 24, 24, 256)
...
#        assert convs.get_shape() == (conf.batch_size, 32*6*6, 8)
        ...
        assert convs.get_shape() == (conf.batch_size, 32*8*8, 8)
    ...
    def digit_caps(self, inputs, num_iters, num_caps, routing=0):
        ...
        # Validate inputs
        assert inputs.get_shape() == (conf.batch_size, 32*8*8, 8)

        # Reshape input
        u = tf.reshape(inputs, [conf.batch_size, 32*8*8, 1, 8, 1])
        u = tf.tile(u, [1, 1, num_caps, 1, 1])  # [batch_size, 32*6*6, num_caps, 8, 1]
        
        # Dynamic routing
        bij = tf.zeros((32*8*8, num_caps), name='b')
        wij = tf.get_variable('wij', shape=(1, 32*8*8, num_caps, 8, 16), initializer=tf.contrib.layers.xavier_initializer())
        ...
        uhat = tf.reshape(uhat, [conf.batch_size, 32*8*8, num_caps, 16])  # [batch_size, 32*6*6, num_caps, 16]
        if routing == 0:
            uhat_stop_grad = tf.stop_gradient(uhat)
            assert uhat.get_shape() == (conf.batch_size, 32*8*8, num_caps, 16)

            for r in range(num_iters):
                with tf.variable_scope('routing_iter_' + str(r)):
                    # cij
                    cij = tf.nn.softmax(bij, dim=-1)  # [32*6*6, num_caps]
                    cij = tf.tile(tf.reshape(cij, [1, 32*8*8, num_caps, 1]),
                                [conf.batch_size, 1, 1, 1])  # [batch_size, 32*6*6, num_caps, 1]
                    assert cij.get_shape() == (conf.batch_size, 32*8*8, num_caps, 1)
                    ...
                    if r == num_iters-1: 
                        ...
                        bij = bij + a
                        assert a.get_shape() == (32*8*8, num_caps)
        if routing == 1:
            bij = tf.get_variable('bij', shape=(32*8*8, num_caps), initializer=tf.constant_initializer(0.0))
    ...
    def reconstruction(self, inputs, targets):
        ...
        with tf.variable_scope('reconstruction'):
            ...
            recons = tf.reshape(fc_sigmoid, shape=(conf.batch_size, datasize, datasize, 3))
